...
public class CoreImageCallback implements SurfaceHolder.Callback {
    ...
    private float calcBaseScale( final SizeInfo srcSize , final SizeInfo dstSize ) {
    ...
    public void setIds( final long[] ids ) {
        System.err.println( "***** setIds" );
        // currently, consider only single id
        _ids = ids;
        ...
        _thumbnailSources = Lists.newArrayList();

        for ( int index = 0 ; index < doc.pages ; index++ ) {
            _sources.add( Kernel.getLocalProvider().getImagePath( id , index , 0 , 0 , 0 ) );
            _thumbnailSources.add( Kernel.getLocalProvider().getThumbnailPath( id , index ) );
        }

        _index = 0;

        _images = new Bitmap[ _sources.size() ];
        _images[ 0 ] = decode( _sources.get( 0 ) );
        _images[ 1 ] = decode( _sources.get( 1 ) );

        _imageSize = new SizeInfo( _images[ 0 ].getWidth() , _images[ 0 ].getHeight() );
        _scale = _minScale = calcBaseScale( _imageSize , _surfaceSize );
        _maxScale = 1;
        _offset = new PointF( 0 , 0 );

        System.err.println( "***** setIds done: " + _imageSize );
