...
import jp.archilogic.docnext.android.core.text.engine.HorizontalCoreTextEngine;
import jp.archilogic.docnext.android.core.text.engine.VerticalCoreTextEngine;
...
public enum TextDocDirection {
    ...
    boolean shouldChangeToNext( final float offset , final Size cache , final Size surface ) {
        switch ( this ) {
        case HORIZONTAL:
    ...
    boolean shouldChangeToPrev( final float offset , final Size cache , final Size surface ) {
        switch ( this ) {
        case HORIZONTAL:
            return offset > 0;
        case VERTICAL:
    ...
    Size toCacheSize( final TextLayoutInfo[] layouts , final Size surface , final CoreTextConfig config ) {
        switch ( this ) {
        case HORIZONTAL:
            final TextLayoutInfo last = layouts[ layouts.length - 1 ];
            return new Size( surface.width , ( int ) Math.ceil( last.y + last.height + config.verticalPadding ) );
        case VERTICAL:
            final TextLayoutInfo first = layouts[ 0 ];
            return new Size( ( int ) Math.ceil( first.x + first.width + config.horizontalPadding ) , surface.height );
        default:
            throw new RuntimeException();
        }
    }

    RectF toDrawRect( final Bitmap[] caches , final Size surface , final int index , final int delta ,
    ...
    private RectF toDrawRectOnHorizontal( final Bitmap[] caches , final Size surface , final int index ,
    ...
    private RectF toDrawRectOnVertical( final Bitmap[] caches , final Size surface , final int index , final int delta ,
            final float pageSpace ) {
        final Bitmap c = caches[ index + delta ];
    ...
    float updateOffset( final float offset , final Size cache , final Size surface , final CoreTextConfig config ,
